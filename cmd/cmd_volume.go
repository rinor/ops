...
func volumeCreateCommandHandler(cmd *cobra.Command, args []string) {
	...
	if sourceImage == cmd.Flag("image").NoOptDefVal {
		...
		log.Infof("volume: created image %s with UUID %s and label %s\n", res.Name, res.ID, res.Label)
		return
	}
	...
	if sourceImage != "" {
		...
		log.Infof("volume: created disk with label %s from image %s\n", name, sourceImage)
		return
	}
	if sourceVolume != "" {
		...
		log.Infof("volume: created disk with label %s from volume %s\n", name, sourceVolume)
		return
	}
	if sourceSnapshot != "" {
		...
		log.Infof("volume: created disk with label %s from snapshot %s\n", name, sourceSnapshot)
		return
	}
